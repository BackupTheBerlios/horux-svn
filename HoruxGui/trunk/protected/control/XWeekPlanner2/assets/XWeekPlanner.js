...
var initTopHour = 7;	// Initially auto scroll scheduler to the position of this hour
...
function editEventWindow(e,inputDiv)
{
	...
	var editEvent = window.open(popupWindowUrl + '&c=' + inputDiv.id,'editEvent','width=500,height=300,status=no,top='+( screen.availHeight / 2 - 150)+',left='+( screen.availWidth / 2 - 250));
	editEvent.focus();
}
...
//	var items = weekSchedule_ajaxObjects[ajaxIndex].response.split(/<item>/g);
	...
	for(var no=1;no<items.length;no++){
		...
		{
			itm = result[j];
			var date = itm.date.split('-');
			var hour = itm.hour.split(':');
			var duration = itm.duration.split(':');
			itm.eventStartDate = new Date(date[0],date[1]-1,date[2],hour[0],hour[1],0,0);
			itm.eventEndDate = new Date(date[0],date[1]-1,date[2],hour[0]-0+(duration[0]-0),hour[1]-0+(duration[1]-0),0,0);

			var dayDiff = itm.eventStartDate.getTime() - dateStartOfWeek.getTime();
			dayDiff = Math.floor(dayDiff / (1000*60*60*24));
			el_x = dayPositionArray[dayDiff];
			topPos = getYPositionFromTime(itm.eventStartDate.getHours(),itm.eventStartDate.getMinutes());

			var elHeight = (itm.eventEndDate.getTime() - itm.eventStartDate.getTime()) / (60 * 60*1000);
			elHeight = Math.round((elHeight * (itemRowHeight + 1)) - 2);

			currentAppointmentDiv = createNewAppointmentDiv((el_x - appointmentsOffsetLeft),topPos,(newAppointmentWidth-(appointmentMarginSize*2)),itm.description,elHeight);
			currentAppointmentDiv.id = itm.id;
			...
			if(itm.bgColorCode && itm.bgColorCode.match(/^#[0-9A-F]{6}$/)){
				currentAppointmentDiv.style.backgroundColor = itm.bgColorCode;
			}
			...
			appointmentProperties[newIndex]['id'] = itm.id;
			appointmentProperties[newIndex]['description'] = itm.description;
			appointmentProperties[newIndex]['bgColorCode'] = itm.bgColorCode;
			appointmentProperties[newIndex]['eventStartDate'] = itm.eventStartDate;
			appointmentProperties[newIndex]['eventEndDate'] = itm.eventEndDate;
...
{
	var date = dateStartOfWeek.getFullYear() + '-' + (dateStartOfWeek.getMonth()+1) + '-' + dateStartOfWeek.getDate();
	new Prado.Callback(weekScheduler_server_id,
						{CommandName:'load', CommandParameter:date}, null,
						{'CausesValidation':false, 'onSuccess': parsePradoResponse});
...
function parsePradoResponse(request,result)
{
//	alert(result[0].description);
	parseItemsFromServer(result);
}
...
//	alert(inputId);
	...
	var startDate = appointmentProperties[inputId]['eventStartDate'];
	var endDate = appointmentProperties[inputId]['eventEndDate'];
	var date = startDate.getFullYear() + '-' + (startDate.getMonth() +1) + '-' + startDate.getDate();
	var hour = startDate.getHours() + ':' + startDate.getMinutes();
	if((endDate.getMinutes() - startDate.getMinutes()) < 0)
		var duration = ((endDate.getHours() - startDate.getHours()) - 1) + ':' + (startDate.getMinutes() - endDate.getMinutes());
	else
		var duration = (endDate.getHours() - startDate.getHours()) + ':' + (endDate.getMinutes() - startDate.getMinutes());

	var saveString = { "id" : appointmentProperties[inputId]['id'],'description' : appointmentProperties[inputId]['description'],
	'date' : date,
	'duration' : duration,
	'hour' : hour};
...
/*
	...
	weekSchedule_ajaxObjects[ajaxIndex].runAJAX();		// Execute AJAX function*/
	new Prado.Callback(weekScheduler_server_id_save,
						{CommandName:'save', CommandParameter:saveString}, null,
						{'CausesValidation':false, 'onSuccess': function(request,response){ saveAnItemToServer_complete(response,appointmentProperties[inputId]['id'])} });

}
function saveAnItemToServer_complete(response,oldId)
{
	if(oldId.indexOf)
	{
		if(oldId.indexOf('new_')>=0){
			appointmentProperties[oldId]['id'] = response;
			appointmentProperties[oldId]['object'].id = response.id;
...
function schedulerKeyboardEvent(e){
	...
	new Prado.Callback(weekScheduler_server_id_delete,
						{CommandName:'save', CommandParameter:activeEventObj.id}, null,
						{'CausesValidation':false, 'onSuccess': deleteEventFromView });
...
function deleteEventFromView(response,result)
{
	if(response){
...
{
	weekScheduler_container = document.getElementById(weekScheduler_container_id);
