...
class modattribution extends Page
{
       ...
       if(!$this->isPostBack)
        {
        	...
        	$cmd->bindParameter(":id",$this->Request["attid"], PDO::PARAM_INT);
        	$query = $cmd->query();			
			$data = $query->read();

			$sql2 = "SELECT * FROM hr_vp_subscription WHERE id=:id";
        	$cmd2 = $this->db->createCommand( $sql2);
        	$cmd2->bindParameter(":id",$data['subcription_id'], PDO::PARAM_INT);
        	$query2 = $cmd2->query();			
			$data2 = $query2->read();			
    ...
    protected function saveData()
    {
		...
		if($this->start->Date != "")
		{
      	...
      	$cmd->bindParameter(":start",$start,PDO::PARAM_STR);
		...
		if($this->start->Date != "")
		{
      	...
      	$cmd->bindParameter(":end",$end, PDO::PARAM_STR);

		$cmd->bindParameter(":status",$this->status->getSelectedValue(), PDO::PARAM_STR);

      	$cmd->bindParameter(":id",$this->Request['attid'], PDO::PARAM_STR);
