...
#include "caccesslinkrs232.h"
#include <QtCore>

CAccessLinkRS232::CAccessLinkRS232(QObject *parent) : QObject(parent)
{
  _isConnected = false;
  input = 0;
  output = 0;
  ...
  addFunction("openDoor", CAccessLinkRS232::s_openDoorLock);

}

CDeviceInterface *CAccessLinkRS232::createInstance (QMap<QString, QVariant> config, QObject *parent )
{
  CDeviceInterface *p = new CAccessLinkRS232 ( parent );
  ...
  p->setParameter("serial_port",config["serial_port"]);
...
void CAccessLinkRS232::deviceAction(QString xml)
{
...
void CAccessLinkRS232::connectChild(CDeviceInterface *)
{

}

QVariant CAccessLinkRS232::getParameter(QString paramName)
{
  if(paramName == "name")
    return name;
  if(paramName == "id")
...
void CAccessLinkRS232::setParameter(QString paramName, QVariant value) 
{
  if(paramName == "name")
    name = value.toString();
  if(paramName == "id")
...
bool CAccessLinkRS232::open()
{
  ...
  port = new QextSerialPort(serial_port);
...
bool CAccessLinkRS232::isOpened()
{
  return _isConnected;
}

void CAccessLinkRS232::appendMessage(uchar *msg, int len)
{
  if(status == FREE)
  {
    if(isOpened())
...
void CAccessLinkRS232::timerEvent(QTimerEvent *e)
{
...
void CAccessLinkRS232::dispatchMessage(QByteArray ba)
{
...
void CAccessLinkRS232::getFirmware()
{
  uchar cmd[3];
...
void CAccessLinkRS232::getSerialNumber()
{
  uchar cmd[3];
...
void CAccessLinkRS232::setAntenna(bool flag)
{
  uchar cmd[4];
...
void CAccessLinkRS232::resetOutput()
{
  uchar cmd[4];
...
void CAccessLinkRS232::getRdStatus()
{
  uchar cmd[3];
...
void CAccessLinkRS232::getOutputStatus()
{
  uchar cmd[3];
...
Q_EXPORT_PLUGIN2(accesslinkrs232, CAccessLinkRS232);




/*!
    \fn CAccessLinkRS232::checkReaderStatus(uchar status)
 */
void CAccessLinkRS232::checkReaderStatus(uchar status)
{
  if(input != status)
  {
    uchar mask = 1;
    ...
    if(tc_new != tc_old)
    {
        ...
        {
            QString xml = CXmlFactory::deviceEvent(QString::number(id), "1015", "The tag control is on");
...
/*!
    \fn CAccessLinkRS232::handleSn(QString sn)
 */
void CAccessLinkRS232::handleSn(QString sn)
{
  ...
  QString xml = CXmlFactory::keyDetection(QString::number(id),QString::number(id),getAccessPluginName(), sn);
...
void CAccessLinkRS232::s_openDoorLock(QObject *p, QMap<QString, QVariant>params)
{

  CAccessLinkRS232 *pThis = qobject_cast<CAccessLinkRS232 *>(p);
...
/*!
    \fn CAccessLinkRS232::logComm(QByteArray ba)
 */
void CAccessLinkRS232::logComm(uchar *ba, bool isReceive, int len)
{
  if(!_isLog)
    return;
...
QDomElement CAccessLinkRS232::getDeviceInfo(QDomDocument xml_info )
{
